...
D3DXVECTOR2 mouseLastPos;
D3DXVECTOR2 mouseNowPos;
bool canCheckSlide;
bool isSlideLeft;
bool isSlideRight;
int slidePerX;
...
HRESULT InitializeMouse(HINSTANCE hInst,HWND hWindow)
{
	...
	//slide
	mouseLastPos = { (float)mouseState.lX,(float)mouseState.lY };
	isSlideLeft = false;
	isSlideRight = false;
	canCheckSlide = true;
	slidePerX = 45;
...
HRESULT UpdateMouse()
{
	...
	//check slide lr
	isSlideLeft = isSlideRight = false;
	if (canCheckSlide)
	{
		if (mouseState.lX < -slidePerX)
		{
			isSlideLeft = true;
			//MyLog("\n\nslide Left\n\n");
		}
		else if (mouseState.lX > slidePerX)
		{
			isSlideRight = true;
			//MyLog("\n\nslide Right\n\n");
		}
	...
	{
		slideFrameCount++;
		if (slideFrameCount >= slideFrameMax)
		{
			slideFrameCount = 0;
			canCheckSlide = true;
		}
...
{
	if (isSlideLeft)canCheckSlide = false;
	return isSlideLeft;
}
bool IsMouseSlideRight()
{
	if (isSlideRight)canCheckSlide = false;
	return isSlideRight;
}
...
//------------------------------------------ 更新
float GY, GX;
bool is_push[GAMEPADMAX][4] = { false };
void UpdatePad(void)
{
	HRESULT			result;
	DIJOYSTATE2		dijs;
	...
	for ( i=0 ; i<padCount ; i++ ) 
	{
		...
		if (dijs.rgdwPOV[0] != 0xFFFFFFFF)
		{
			...
			switch (dijs.rgdwPOV[0])
			{
				// 上
			case 0:
				is_push[i][ten_up] = true;
				break;
				// 右上
			case 4500:
				/*is_push[i][ten_right] = true;
				is_push[i][ten_up] = true;*/
				break;
				// 右
			case 9000:
...
{
	//bool pressed = is_push[padNo][tenType];
	//is_push[padNo][tenType] = false;
	return is_push[padNo][tenType];
}
...
void TestPad()
{
	if (IsButtonTriggered(GetPadID(),rgbButtons_X))
	{
		MyLog("btn 0 \n");
	}
	if (IsButtonTriggered(GetPadID(), rgbButtons_A))
	{
		MyLog("btn 1 \n");
	}
	if (IsButtonTriggered(GetPadID(), rgbButtons_A))
	...
	if (IsButtonTriggered(GetPadID(), BUTTON_A))
	{
		MyLog("btn 0 \n");
	}
	if (IsButtonTriggered(GetPadID(), rgbButtons_A))
